...
import (
	...
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/csync"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/pubsub"
	"github.com/google/uuid"
)

// Mode is the session-wide permission posture, matching Claude Code's
// editing modes. It is orthogonal to the global skip ("yolo") flag: yolo, when
// set, bypasses everything regardless of Mode.
type Mode int32

const (
	// ModeNormal prompts for every tool call that needs permission.
	ModeNormal Mode = iota
	// ModeAcceptEdits auto-approves in-workspace file-edit tools
	// (edit/write/multiedit) but still prompts for everything else
	// (e.g. bash, downloads).
	ModeAcceptEdits
	// ModePlan is read-only: every workspace-mutating tool (edits, bash,
	// downloads) is blocked so the agent researches and proposes a plan
...
// Next returns the mode reached by cycling (normal → accept-edits → plan →
// normal), the behavior bound to Shift+Tab in the TUI.
...
// editToolNames are the in-workspace file-edit tools auto-approved under
// ModeAcceptEdits. The values mirror the ToolName constants in
// agent/tools (duplicated here to avoid a permission→tools import cycle).
...
// planBlockedToolNames are the tools blocked under ModePlan: the edit tools
// plus the ones that execute commands or write to disk.
var planBlockedToolNames = map[string]bool{
	"edit":      true,
	"write":     true,
	"multiedit": true,
	"bash":      true,
...
func isPlanBlocked(name string) bool { return planBlockedToolNames[name] }
...
type Service interface {
	...
	SkipRequests() bool
	// SetMode sets the session-wide permission posture (see Mode).
	SetMode(mode Mode)
	// Mode reports the current permission posture.
	Mode() Mode
...
type permissionService struct {
	*pubsub.Broker[PermissionRequest]
	...
	skip                  atomic.Bool
	mode                  atomic.Int32 // permission.Mode
	allowedTools          []string
...
func (s *permissionService) Request(ctx context.Context, opts CreatePermissionRequest) (bool, error) {
	...
	// Editing-mode posture (Claude Code parity). Evaluated before the allowlist
	// so plan mode is authoritative: a mutating tool stays blocked even if it
	// would otherwise be auto-allowed.
	switch Mode(s.mode.Load()) {
	case ModePlan:
		if isPlanBlocked(opts.ToolName) {
			// Tell subscribers the call was denied, then report it as a
			// declined permission so the agent stops mutating and proposes a
			// plan instead of changing anything.
			s.notificationBroker.Publish(pubsub.CreatedEvent, PermissionNotification{
