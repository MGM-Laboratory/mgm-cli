...
import (
	...
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/env"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/filepathext"
...
func (c *Config) configureProviders(store *ConfigStore, env env.Env, resolver VariableResolver, knownProviders []catwalk.Provider) error {
	...
	// Megumi lockdown (defense-in-depth): when the embedding mgm CLI sets
	// MEGUMI_LOCK=1, force default providers off and strip any provider other
	// than the broker so no non-Megumi provider can ever be reached, even if a
	// stray project config tries to add one.
	if os.Getenv("MEGUMI_LOCK") == "1" {
		c.Options.DisableDefaultProviders = true
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if len(c.Options.GlobalContextPaths) == 0 {
		...
		c.Options.GlobalContextPaths = []string{
			filepath.Join(crushConfigDir, "MEGUMI.md"),
