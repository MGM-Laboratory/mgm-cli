...
import (
	...
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/oauth"
...
type ClientWorkspace struct {
	...
	// permMode tracks the editing posture for the TUI. The client/server
	// architecture is opt-in (CRUSH_CLIENT_SERVER) and Megumi never enables it,
	// so the mode is kept client-side only; server-side enforcement for this
	// path is not wired (see PHASE4 status doc). Plan/accept-edits enforcement
	// is fully wired for the default in-process path (AppWorkspace).
