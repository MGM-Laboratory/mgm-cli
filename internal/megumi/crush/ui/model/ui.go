...
import (
	...
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/agent"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/agent/hyper"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/agent/notify"
	agenttools "github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/agent/tools"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/agent/tools/mcp"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/app"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/commands"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/config"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/fsext"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/history"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/home"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/message"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/permission"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/pubsub"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/session"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/skills"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/stringext"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/anim"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/attachments"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/chat"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/common"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/completions"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/dialog"
	fimage "github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/image"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/notification"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/styles"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/util"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/version"
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/workspace"
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case uiFocusEditor:
		...
		} else if ph := modePlaceholder(m.com.Workspace.PermissionMode()); ph != "" {
			m.textarea.Placeholder = ph
		}
	}
...
func (m *UI) handleDialogMsg(msg tea.Msg) tea.Cmd {
	...
	case dialog.ActionToggleYoloMode:
		...
		m.dialog.CloseDialog(dialog.CommandsID)
	case dialog.ActionSetPermissionMode:
		m.com.Workspace.PermissionSetMode(msg.Mode)
		cmds = append(cmds, util.ReportInfo("Mode: "+modeLabel(msg.Mode)))
		m.dialog.CloseDialog(dialog.CommandsID)
	...
	case dialog.ActionSelectReasoningEffort:
		...
		// Megumi: the effort tier is sent to the broker via the x-megumi-effort
		// header (the broker maps it to a thinking-token budget). We set the live
		// tier here rather than mutating the model's client-side reasoning, so the
		// broker stays authoritative for thinking.
		agent.SetEffort(msg.Effort)
		cmds = append(cmds, util.ReportInfo("Effort set to "+msg.Effort))
...
func (m *UI) handleKeyPressMsg(msg tea.KeyPressMsg) tea.Cmd {
	...
	handleGlobalKeys := func(msg tea.KeyPressMsg) bool {
		...
		case key.Matches(msg, m.keyMap.CycleMode):
			next := m.com.Workspace.PermissionMode().Next()
			m.com.Workspace.PermissionSetMode(next)
			cmds = append(cmds, util.ReportInfo("Mode: "+modeLabel(next)))
			return true
		}
		return false
	}
...
// modeLabel is the short label reported when cycling editing modes.
func modeLabel(mode permission.Mode) string {
	switch mode {
	case permission.ModeAcceptEdits:
		return "accept edits (file edits auto-approved)"
	case permission.ModePlan:
		return "plan (read-only — no edits or commands)"
	default:
		return "normal"
	}
}

// modePlaceholder is the editor placeholder shown while a non-default editing
// mode is active (empty for normal mode, which falls back to the usual
// placeholder).
func modePlaceholder(mode permission.Mode) string {
	switch mode {
	case permission.ModeAcceptEdits:
		return "accept-edits mode — file edits auto-approved · shift+tab to cycle"
	case permission.ModePlan:
		return "plan mode — read-only, no edits or commands · shift+tab to cycle"
	default:
		return ""
	}
}
...
func renderLogo(t *styles.Styles, compact, hyper bool, width int) string {
	// Megumi brand wordmark (replaces the upstream CRUSH logo), gradient-colored
	// from the theme's logo colors. RenderInline falls back to a compact wordmark
	// when the pane is too narrow for the full art.
	return banner.RenderInline(width, t.Logo.TitleColorA, t.Logo.TitleColorB)
}
