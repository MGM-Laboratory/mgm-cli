...
import (
	...
	"github.com/MGM-Laboratory/mgm-cli/internal/megumi/crush/ui/list"
...
func (c *Commands) defaultCommands() []*CommandItem {
	commands := []*CommandItem{
		// "clear" aliases New Session to match Claude Code's /clear (start a
		// fresh conversation).
		NewCommandItem(c.com.Styles, "new_session", "New Session", "ctrl+n", ActionNewSession{}).WithAliases("clear", "new"),
		NewCommandItem(c.com.Styles, "switch_session", "Sessions", "ctrl+s", ActionOpenDialog{SessionsID}),
		NewCommandItem(c.com.Styles, "switch_model", "Switch Model", "ctrl+l", ActionOpenDialog{ModelsID}).WithAliases("model"),
	}
	...
	if agentCfg, ok := cfg.Agents[config.AgentCoder]; ok {
		...
		if providerCfg != nil && model != nil && model.CanReason {
			...
			// Effort picker (Megumi): tiers map to the broker's thinking budget.
			if len(model.ReasoningLevels) > 0 {
				commands = append(commands, NewCommandItem(c.com.Styles, "select_effort", "Effort", "", ActionOpenDialog{
	...
	commands = append(
		commands,
		// Editing modes (Claude Code parity). shift+tab cycles
		// normal → accept-edits → plan; these palette entries jump directly.
		NewCommandItem(c.com.Styles, "mode_normal", "Normal Mode", "", ActionSetPermissionMode{Mode: permission.ModeNormal}).WithAliases("normal mode"),
		NewCommandItem(c.com.Styles, "mode_accept_edits", "Auto-Accept Edits Mode", "", ActionSetPermissionMode{Mode: permission.ModeAcceptEdits}).WithAliases("accept edits", "auto accept"),
		NewCommandItem(c.com.Styles, "mode_plan", "Plan Mode", "shift+tab", ActionSetPermissionMode{Mode: permission.ModePlan}).WithAliases("plan"),
		NewCommandItem(c.com.Styles, "toggle_yolo", "Toggle Yolo Mode", "ctrl+y", ActionToggleYoloMode{}),
		NewCommandItem(c.com.Styles, "toggle_help", "Toggle Help", "ctrl+g", ActionToggleHelp{}).WithAliases("help"),
