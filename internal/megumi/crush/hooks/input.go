...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	...
	// Each hook variable is exported under both the MEGUMI_ (Megumi-branded,
	// preferred for new hook scripts) and CRUSH_ (upstream, kept for
	// compatibility) prefixes.
	addVar := func(name, value string) {
		env = append(env,
			fmt.Sprintf("MEGUMI_%s=%s", name, value),
			fmt.Sprintf("CRUSH_%s=%s", name, value),
		)
	}
	addVar("EVENT", eventName)
	addVar("TOOL_NAME", toolName)
	addVar("SESSION_ID", sessionID)
	addVar("CWD", cwd)
	addVar("PROJECT_DIR", projectDir)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			addVar("TOOL_INPUT_COMMAND", cmd.String())
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			addVar("TOOL_INPUT_FILE_PATH", fp.String())
		}
	}
